...
async function processVote(message: string, mqttClient: MqttClient) {
	...
	const publicKey = await jose.importJWK(JSON.parse(env.JWT_PUBLIC_KEY), 'RS256'); // Replace 'RS256' with the correct algorithm if different
	
	// Verify the JWT signature
	await jose.jwtVerify(voteKey, publicKey);
